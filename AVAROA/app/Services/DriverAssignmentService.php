...
class DriverAssignmentService
{
     ...
     * Customer requests 'moto' → matches drivers with moto, moto_veloz, moto_taxi, etc.
     * Legacy drivers with old subtypes will still receive notifications.
     */
    ...
    protected function sendRequestToDriver(Driver $driver, Trip $trip, string $language = 'es'): void
    {
        try {
            ...
            if (!$sent) {
                ...
                $message = $this->buildDriverMessage($trip, $customer, $vehicleInfo);
    ...
    protected function buildDriverMessage(Trip $trip, $customer, string $vehicleInfo): string
    {
        $instructions = $trip->notes ?: 'Ninguna';
        $pickup = $trip->origin_url ?? 'No especificado';
        $destination = $trip->destination_url ?? 'No especificado';
        ...
        return "📢 NUEVA SOLICITUD DE ENTREGA\n" .
    ...
    protected function notifyCustomerDriverAssigned(Trip $trip, Driver $driver, ?Vehicle $vehicle): void
    {
        ...
        $message = "✅ *¡Mensajero asignado!*\n\n" .
            ...
            "💰 *Precio:* {$priceFormatted}\n\n" .
            "🚚 El mensajero ya va hacia tu ubicación de recogida.";
